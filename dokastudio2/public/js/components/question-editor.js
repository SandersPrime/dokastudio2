...
const QuestionEditorComponent = {
  ...
  renderAnswersEditor(answers = []) {
    ...
    container.innerHTML = answers.map((answer, index) => `
      <div class="answer-item ${answer.isCorrect ? 'correct' : ''}" data-index="${index}">
        <div class="answer-row-shell">
          <button type="button" class="answer-drag-handle drag-handle" title="Перетащить ответ" aria-label="Перетащить ответ">≡</button>
          ...
          >
          <button type="button" class="answer-correct-toggle ${answer.isCorrect ? 'is-on' : ''}" onclick="setCorrectAnswer(${index})" aria-label="Отметить как правильный">
            ${answer.isCorrect ? 'Верный' : 'Верный?'}
          </button>
          <button type="button" class="answer-remove-btn" onclick="removeAnswer(${index})" aria-label="Удалить ответ">×</button>
        </div>
      </div>
    `).join('');
  },
  ...
  renderStudioPreview(question) {
    ...
    const backgroundType = appearance.backgroundType || (question.backgroundImageUrl ? 'image' : (appearance.backgroundGradient ? 'gradient' : 'color'));
    const backgroundImage = backgroundType === 'gradient'
      ? (appearance.backgroundGradient || `linear-gradient(${appearance.backgroundGradientDirection || '135deg'}, ${appearance.backgroundGradientStart || '#f6f8fb'}, ${appearance.backgroundGradientEnd || '#dfe7ff'})`)
      : (backgroundType === 'image' && question.backgroundImageUrl ? `url("${question.backgroundImageUrl}")` : 'none');
    const answerStyle = appearance.answerCardStyle || 'rounded';
    const answerRadius = answerStyle === 'square' ? '4px' : (answerStyle === 'soft' ? '18px' : '10px');
    const answerShadow = answerStyle === 'soft'
      ? '0 14px 34px rgba(15, 23, 42, 0.14)'
      : (answerStyle === 'square' ? 'none' : '0 10px 24px rgba(15, 23, 42, 0.08)');
    const contentAlignMap = { stretch: 'stretch', start: 'flex-start', center: 'center' };
    const contentVerticalMap = { start: 'flex-start', center: 'center', end: 'flex-end' };
    ...
    canvas.style.setProperty('--studio-background-fit', appearance.backgroundImageFit || 'cover');
    canvas.style.setProperty('--studio-background-video-fit', appearance.backgroundImageFit || 'cover');
    canvas.style.setProperty('--studio-overlay-color', appearance.backgroundOverlayColor || '#000000');
    canvas.style.setProperty('--studio-overlay-opacity', String((appearance.backgroundOverlayOpacity || 0) / 100));
    ...
    canvas.style.setProperty('--studio-answer-card-bg', this.hexToRgba(appearance.answerCardColor || '#ffffff', (appearance.answerCardOpacity || 92) / 100));
    canvas.style.setProperty('--studio-answer-radius', answerRadius);
    canvas.style.setProperty('--studio-answer-shadow', answerShadow);
    canvas.style.setProperty('--studio-answer-border-width', `${appearance.answerAccentWeight ?? 1}px`);
    canvas.style.setProperty('--studio-answer-gap', `${appearance.answerGap || 12}px`);
    ...
    canvas.style.setProperty('--studio-question-weight', appearance.questionBold ? '800' : '700');
    canvas.style.setProperty('--studio-question-style', appearance.questionItalic ? 'italic' : 'normal');
    canvas.style.setProperty('--studio-question-decoration', appearance.questionUnderline ? 'underline' : 'none');
    canvas.style.setProperty('--studio-answer-weight', appearance.answerBold ? '800' : '700');
    canvas.style.setProperty('--studio-answer-style', appearance.answerItalic ? 'italic' : 'normal');
    canvas.style.setProperty('--studio-answer-decoration', appearance.answerUnderline ? 'underline' : 'none');
    canvas.style.setProperty('--studio-question-align', appearance.questionAlign || appearance.textAlign || 'left');
    canvas.style.setProperty('--studio-answer-align', appearance.answerAlign || appearance.textAlign || 'left');
    canvas.style.setProperty('--studio-text-align', appearance.questionAlign || appearance.textAlign || 'left');
    canvas.style.setProperty('--studio-content-align', contentAlignMap[appearance.contentAlign] || 'stretch');
    canvas.style.setProperty('--studio-content-vertical', contentVerticalMap[appearance.contentVertical] || 'center');
    this.syncBackgroundVideo(canvas, appearance, backgroundType);

    title.textContent = question.text || 'Введите текст вопроса';
    if (subtitle) subtitle.textContent = question.subtitle || '';
    if (subtitle) subtitle.style.textAlign = appearance.questionAlign || appearance.textAlign || 'left';
    ...
    answersGrid.innerHTML = hideAnswers ? '' : answers.map((answer, index) => `
      <button class="preview-answer ${answer.isCorrect ? 'correct is-correct' : 'is-distractor'}" type="button">
  ...
  syncBackgroundVideo(canvas, appearance, backgroundType) {
    let video = canvas.querySelector('.slide-background-video');
    const url = appearance.backgroundVideoUrl;

    if (backgroundType !== 'video' || !url) {
    ...
    video.muted = appearance.backgroundVideoMuted !== false;
    video.loop = appearance.backgroundVideoLoop !== false;
  ...
  hexToRgba(hex, opacity = 1) {
    ...
    if (Number.isNaN(intValue)) return `rgba(255, 255, 255, ${opacity})`;
