...
const QuestionListComponent = {
  ...
  renderQuestionsList(questions, currentQuestionIndex = -1, editFunctionName = 'editQuestion') {
    ...
    const renderedItems = items.map((question, index) => {
      ...
      const meta = `${question.points ?? 0} б · ${question.timeLimit ?? 30} с`;
      const answers = Array.isArray(question.answers) ? question.answers.slice(0, 4) : [];
      const correctCount = Array.isArray(question.answers) ? question.answers.filter((answer) => answer.isCorrect).length : 0;
      ...
      return `
        <div class="question-item slide-thumb ${isActive ? 'active' : ''}" data-id="${question.id || question.tempId}" draggable="true" tabindex="0" aria-label="${this.escapeHtml(text)}" onkeydown="if(event.key === 'Enter' || event.key === ' ') { event.preventDefault(); ${editFunctionName}(${index}); }">
          ...
          <div class="slide-thumb-meta" onclick="${editFunctionName}(${index})">
            ...
            <small>${meta}${correctCount ? ` · ${correctCount} верн.` : ''}</small>
          </div>
          <div class="slide-thumb-tools">
            <button type="button" class="outline-icon-button drag-handle" title="Перетащить" aria-label="Перетащить">≡</button>
            <button type="button" class="outline-icon-button duplicate" onclick="duplicateQuestion(${index})" title="Дублировать" aria-label="Дублировать">⧉</button>
            <button type="button" class="outline-icon-button danger" onclick="${editFunctionName}(${index}); deleteCurrentQuestion()" aria-label="Удалить">×</button>
          </div>
        </div>
      `;
    }).join('');
    ...
    container.innerHTML = `
      ${renderedItems}
      <button type="button" class="outline-add-slide" onclick="addQuizElement('QUESTION')" aria-label="Добавить новый слайд">
  ...
  getElementIcon(elementType, slideType) {
    ...
    if (slideType === 'ROUND_END') return '✓';
    if (slideType === 'WAGER') return '₽';
    if (slideType === 'AUDIENCE_RESPONSE') return '◔';
    if (slideType === 'TRIVIA_BOARD') return '▦';
    if (slideType === 'TRIVIA_LADDER') return '↟';
    if (slideType === 'TRIVIA_FEUD') return '100';
    if (slideType === 'BINGO') return '□';
    if (slideType === 'GAME_MODULE') return '◆';
  ...
  getQuestionTypeLabel(type) {
    const labels = {
      ...
      MULTIPLE_CORRECT_SINGLE_PICK: 'Несколько верных',
      MULTIPLE_CORRECT_MULTI_PICK: 'Несколько верных',
