...
const QuestionEditorService = {
  ...
  collectAppearance(document) {
    return {
      ...
      titleColor: this.getValue(document, 'questionColor', this.getValue(document, 'titleColor', '#111827')),
      answerColor: this.getValue(document, 'answerColor', '#111827'),
      answerChipColor: this.getValue(document, 'answerAccentColor', this.getValue(document, 'answerChipColor', '#6a4fff')),
      answerCardColor: this.getValue(document, 'answerCardColor', '#ffffff'),
      answerCardStyle: this.getValue(document, 'answerCardStyle', 'rounded'),
      answerAccentWeight: this.getIntValue(document, 'answerBorderWeight', this.getIntValue(document, 'answerAccentWeight', 1)),
      answerGap: this.getIntValue(document, 'answerGap', 12),
      answerCardOpacity: this.getIntValue(document, 'answerCardOpacity', 92),
      ...
      questionBold: this.getChecked(document, 'questionBold', false),
      questionItalic: this.getChecked(document, 'questionItalic', false),
      questionUnderline: this.getChecked(document, 'questionUnderline', false),
      answerBold: this.getChecked(document, 'answerBold', false),
      answerItalic: this.getChecked(document, 'answerItalic', false),
      answerUnderline: this.getChecked(document, 'answerUnderline', false),
      questionAlign: this.getValue(document, 'questionAlign', this.getValue(document, 'textAlign', 'left')),
      answerAlign: this.getValue(document, 'answerAlign', this.getValue(document, 'textAlign', 'left')),
      textAlign: this.getValue(document, 'questionAlign', this.getValue(document, 'textAlign', 'left')),
      backgroundType: this.getValue(document, 'backgroundType', 'color'),
      backgroundGradientStart: this.getValue(document, 'backgroundGradientStart', '#f6f8fb'),
      backgroundGradientEnd: this.getValue(document, 'backgroundGradientEnd', '#dfe7ff'),
      backgroundGradientDirection: this.getValue(document, 'backgroundGradientDirection', '135deg'),
      backgroundGradient: this.getBackgroundGradient(document),
      backgroundImageFit: this.getValue(document, 'backgroundImageFit', 'cover'),
      backgroundVideoUrl: this.getValue(document, 'backgroundVideoUrl', ''),
      backgroundVideoMuted: this.getChecked(document, 'backgroundVideoMuted', true),
      backgroundVideoLoop: this.getChecked(document, 'backgroundVideoLoop', true),
      backgroundOverlayColor: this.getValue(document, 'backgroundOverlayColor', '#000000'),
      backgroundOverlayOpacity: this.getIntValue(document, 'backgroundOverlayOpacity', 0),
      contentAlign: this.getValue(document, 'contentAlign', 'stretch'),
      contentVertical: this.getValue(document, 'contentVertical', 'center'),
      titleSize: this.getIntValue(document, 'questionSize', this.getIntValue(document, 'titleSize', 48)),
      answerSize: this.getIntValue(document, 'answerSize', 18),
      answerLayout: this.getValue(document, 'answerLayout', 'grid-2'),
    };
  },

  getBackgroundGradient(document) {
    const legacy = this.getValue(document, 'backgroundGradient', '');
    const start = this.getValue(document, 'backgroundGradientStart', '');
    const end = this.getValue(document, 'backgroundGradientEnd', '');
    const direction = this.getValue(document, 'backgroundGradientDirection', '135deg');
    if (start && end) return `linear-gradient(${direction}, ${start}, ${end})`;
    return legacy;
  },
  ...
  getChecked(document, id, fallback = false) {
    const n = document.getElementById(id);
    if (!n) return fallback;
    if (n.type === 'checkbox') return Boolean(n.checked);
    return n.value === 'true' || n.classList.contains('is-active') || n.getAttribute('aria-pressed') === 'true';
  },
};
